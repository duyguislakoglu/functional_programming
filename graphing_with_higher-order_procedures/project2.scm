...
;;;;;;;::;;:;;::;:;;::::;;::::;::;;::;;;;:::;:;::::;::::::;;:::::;::;;:::;:;:;::;;;:;::;:;:;::;;;:;::;;;:;:::;;:;::;;;;
;;;  	  	   	   
;;; dislakoglu15@ku.edu.tr     Thu Oct 19 13:48:33 2017
;;;  	  	   	   
;;; Before you start:  	  	   	   
;;;  	  	   	   
...
(sqrt (+ (square (- (x-of (start-seg seg)) (x-of (end-seg seg)))) (square (- (y-of (start-seg seg)) (y-of (end-seg seg)))))))	   	   
...
(define (perimeter lst)  	  	   	   
  (if(null? lst) 0
     (+ (segment-length (car lst)) (perimeter (cdr lst)) 
     ) 	  	   	   
))  	  	   	   
...
(define (recursive-make-seg list)
    (if (null? (cdr list)) '()
        (cons (make-seg (car list) (cadr list))
                (recursive-make-seg (cdr list)))
        ))
(recursive-make-seg (append ptlist (cons (car ptlist) '()))) 
)
...
(define (centroid lst)
  (define (number-of-points lst)
    (if (null? lst) 0
        (+ 1 (number-of-points (cdr lst)))
        )
    )
  ...
  (if (null? lst) (make-point 0 0)
      (if (null? lst)
          ...
          (make-point
           (/
            (x-of (add-all-points lst))
            (number-of-points lst)
           )

           (/
            (y-of (add-all-points lst))
            (number-of-points lst)
           )))))
...
(define (add-all-x-s lst)
(if (null? lst) 0
    (+ (x-of (car lst)) (add-all-x-s (cdr lst))) 	  	   	   
))

(define (add-all-y-s lst)
(if (null? lst) 0
    (+ (y-of (car lst)) (add-all-y-s (cdr lst))) 	  	   	   
))

(make-point (/ (add-all-x-s lst) (length lst))
            (/ (add-all-y-s lst) (length lst))
...
;square procedure takes number as an input. cos is another procedure and
;that is why square cannot have "cos" as an input.
;If we evaluated this definition, Scheme responds as the following:
...
; (Number,Number->Number)->(A->Number) where A is type of x. ( type of f: A-> Number type of g: A-> Number )
...
;(square-a-procedure deriv) returns (lambda (x) (square (deriv x)))
;deriv procedure needs a procedure as an input and here x can be any type.
;That is why this definition does not work.
;Scheme responds with "contract violation".
...
;(A-> Number) -> (A -> (Number->Number)) where A is the type of f.
...
;For n=27 they will return the same value.  	  	   	   
...
(lambda (t)(make-point (+ (x-of pt) (* t len (sin (/ pi 4))))
                       (+ (y-of pt) (* t len (sin (/ pi 4))))
)))  	  	   	   
  	  	   	   
;    3. What is the type of diagonal-line?
  	  	   	   
; Point,Number -> (Unit-Interval -> Point)	  	   	   
  	  	   	   
;    4. Show some examples that test your procedure, and that show it
;       returns the correct value.
  	  	   	   
((diagonal-line p1 (* 5 (sqrt 2))) (/ 1 2)) ;(2.5 2.5)
((diagonal-line p1 (* 10 (sqrt 2))) 1) ;(10.0 10.0)
((diagonal-line p1 (* 4 (sqrt 2))) (/ 1 4)) ;(1.0 1.0)
...
(define (upside-down curve)
 (lambda (t)
   (((translate 0 (- (* 2 (y-of (curve t))))) curve) t)
))
...
(define (connect-ends c1 c2)
  (lambda (t) 
    (if (<= t 0.5)
        (c1 (* t 2))
        (make-point (+ (x-of (c1 1))
                       (- (x-of (c2 (- (* t 2) 1))) (x-of (c2 0))))
                    (+ (y-of (c1 1))
                       (- (y-of (c2 (- (* t 2) 1))) (y-of (c2 0))))))
  )
)
	   
; test  	  	   	   
;((connect-ends (diagonal-line p1 (* 5 (sqrt 2)))
;               (diagonal-line p2 (* 5 (sqrt 2))))
; 1)  	  	   	   
; Value: (10 10)  	  	   	   
...
;They are like same curves.

;((draw-points-on g1 200) unit-circle)
;((draw-points-on g2 200) alternative-unit-circle)

;The distribution of the points are not the same since corresponding values for particular t differs due to definitions.
;For example (sin (* 2pi t)) and (sin (* 2pi (square t))) are not same for t=1/4.

;; When we connect the points, distribution of points will not be seen clearly
;;because we are not able to understand the which ones are points or which ones are needed pieces for connection.

;((draw-points-squeezed-to-window g1 200) unit-circle)
;((draw-points-squeezed-to-window g2 200) alternative-unit-circle)
;In unit-circle points are uniformly distributed but in alternative-unit-circle we see that points will appear more frequently as t goes 1. 
...
;***EXAMPLES***
;((draw-connected g1 50) (diagonal-line p1 (* 5 (sqrt 2))))
;filename: Exercise14CurveG1
;((draw-points-on g2 90) (diagonal-line p1 (* 3 (sqrt 2))))
;filename: Exercise14CurveG2
;((draw-points-on g3 200) unit-circle)
;filename: Exercise14CurveG3
...
((squeeze-rectangular-portion -.5 1.5 -.5 1.5) ((repeated gosperize level) initial-curve) )
))  	  	   	   
...
;; EXAMPLES
;(show-points-gosper g1 5 500 unit-line)
;(show-points-gosper g2 2 400 (diagonal-line p1 (* 5 (sqrt 2))))
;(show-points-gosper g3 2 500 arc)
;;
...
(define (param-gosperize theta)
  ...
  (connect-ends
   ...
   ((rotate-around-origin (* -1 theta)) curve)
))))

;((draw-points-on g1 500) ((param-gosperize 45) unit-line))
...
(define (param-gosperize-test1 level)
  ((repeated (param-gosperize (/ pi (+ level 2))) level)
   unit-line))

;((draw-points-on g1 500) (param-gosperize-test1 4))
;filename: Exercise18Curve1.png

(define (param-gosperize-test2 level)
  ((repeated (param-gosperize (/ pi (expt 1.3 level))) level)
   unit-line))

;((draw-points-on g2 500) (param-gosperize-test2 5))
;filename: Exercise18Curve2.png

(define (param-gosperize-test3 level)
  ((repeated (param-gosperize (/ pi level)) level)
   unit-line))

;((draw-points-on g3 500) (param-gosperize-test3 6))
;filename: Exercise18Curve3.png
...
(define (kochize level curve)
  (define (make-koch curve)
    (define rotated-part (connect-ends
                          ((scale (/ 1 3)) ((rotate-around-origin (/ pi 3)) curve))
                          ((scale (/ 1 3)) ((rotate-around-origin (* -1 (/ pi 3))) curve)
                                           )))
    (connect-ends (connect-ends ((scale (/ 1 3)) curve) rotated-part) ((scale (/ 1 3)) curve))
    )
  
  ((repeated make-koch level) curve)
  )
...
;((draw-connected g1 500) (kochize 3 unit-line))
;filename: Exercise20g1.png

;((draw-connected g2 500) (kochize 4 unit-line))
;filename: Exercise20g2.png

;((draw-connected g3 500) (kochize 5 unit-line))
;filename: Exercise20g3.png
...
;; MY EXAMPLE
;((draw-connected g3 500) (kochize 5 (connect-ends (diagonal-line p1 (* 7 (sqrt 2)))
;                                                  unit-circle)))
;filename: Exercise21.png
